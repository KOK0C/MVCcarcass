...
class Car extends Model
{
    ...
    {
        return $this->iconDir . str_replace(' ', '_', strtolower($this->brand->name)) . '/' . $this->icon;
    }

    public function getImage(): string
    {
        return $this->carImgDir . $this->img;
    }
    ...
    {
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . $this->getIcon())) {
            unlink($_SERVER['DOCUMENT_ROOT'] . $this->getIcon());
        }
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . $this->getImage())) {
            unlink($_SERVER['DOCUMENT_ROOT'] . $this->getImage());
        }
        $reviews = Review::findReviewsByModel(0, $this->brand->name, $this->model);
        if ($reviews) {
